...
class ODE_3_Repository:
    ...
    class Label(Group):
        ...
        def iter_linear(self):
            for in_f in self.linear_feature_dimensions:
                for out_f in self.linear_feature_dimensions:
                    yield ODE_3_Repository.Linear(in_features=in_f, out_features=out_f, bias=True)
                    yield ODE_3_Repository.Linear(in_features=in_f, out_features=out_f, bias=False)

        def iter_sigmoid(self):
            yield ODE_3_Repository.Sigmoid()

        def iter_relu(self):
            yield ODE_3_Repository.ReLu(inplace=False)
            #yield ODE_3_Repository.ReLu(inplace=True)  # inplace not supported here
        ...
        def iter_cos(self):
            yield ODE_3_Repository.Cos()

        def iter_sum(self):
            for c in self.constant_values:
                yield ODE_3_Repository.Sum(c)

        def iter_product(self):
            for c in self.constant_values:
                if c != 0:
                    yield ODE_3_Repository.Product(c)

        def iter_copy(self):
            for o in self.dimensions:
                yield ODE_3_Repository.Copy(out_dimension=o)
        ...
        def __contains__(self, item):
            if isinstance(item, ODE_3_Repository.Linear):
            ...
            elif isinstance(item, ODE_3_Repository.Sigmoid):
                return True
            elif isinstance(item, ODE_3_Repository.ReLu):
                return isinstance(item.inplace, bool)
            elif isinstance(item, ODE_3_Repository.Tanh):
                return True
            elif isinstance(item, ODE_3_Repository.Sin):
                return True
            ...
            if isinstance(item, ODE_3_Repository.Copy):
    ...
    class LossFunction(Group):
        ...
        def iter_mseloss(self):
            yield ODE_3_Repository.MSEloss(reduction="mean")
            yield ODE_3_Repository.MSEloss(reduction="sum")
            #yield ODE_3_Repository.MSEloss(reduction="none")

        def iter_l1loss(self):
            yield ODE_3_Repository.L1Loss(reduction="mean")
            yield ODE_3_Repository.L1Loss(reduction="sum")
            #yield ODE_3_Repository.L1Loss(reduction="none")
        ...
        def __contains__(self, value):
            return (value is None or ((isinstance(value, ODE_3_Repository.MSEloss) or isinstance(value, ODE_3_Repository.L1Loss))
    ...
    def specification(self):
        ...
        return {
            ...
            .parameter("l", labels, lambda v: list(labels.iter_cos()))
            .parameter("i", dimension)
            .parameter("o", dimension, lambda v: [v["i"]])
            .parameter("para1", para_labels, lambda v: [(v["l"], v["i"], v["o"])])
            .parameter("para2", para_labels, lambda v: [(v["l"], v["i"], None)])
    ...
    def pretty_term_algebra(self):
        return {
            ...
            "sin": (lambda l, i, o, para1, para2, para3, para4, para5, para6, para7: f"({str(l)}, {i}, {o})"),
...
""")
    ...
    def edgelist_algebra(self):
        return {
            ...
            "cos": (lambda l, i, o, para1, para2, para3, para4, para5, para6, para7: lambda id, inputs: (
                [(x, str((l, i, o)) + str(id)) for x in inputs], [str((l, i, o)) + str(id) for _ in range(0, o)],
                {str((l, i, o)) + str(id): id})),
...
""" for k in range(o)]), tuple(f"x_{id[0]}_{id[1]}_{k}" for k in range(o)))),
        ...
        """ for k in range(o)]), tuple(f"x_{id[0]}_{id[1]}_{k}" for k in range(o)))),
            ...
            "cos": (lambda l, i, o, para1, para2, para3, para4, para5, para6, para7: lambda id, inputs: (f"""
                                self.tanh_{id[0]}_{id[1]} = nn.Tanh()
                        """, "\n".join([f"""
                                x_{id[0]}_{id[1]}_{k} = self.tanh_{id[0]}_{id[1]}({", ".join(inputs)})
                        """ for k in range(o)]), tuple(f"x_{id[0]}_{id[1]}_{k}" for k in range(o)))),
...
""")
    ...
    class SynthSin(nn.Module):
        ...
        def forward(self, x):
            #x = torch.cat(x, dim=1)
            if len(x[1]) != self.o:
                raise ValueError(f"Sin expected {self.o} inputs, but got {len(x)}")
            y = torch.sin(x)
            if len(y[1]) != self.o:
                raise ValueError(f"Sin expected to produce {self.o} outputs, but got {len(y)}")
            return y

    class SynthCos(nn.Module):
        def __init__(self, output_dim):
            super().__init__()
            self.o = output_dim
    ...
    def pytorch_function_algebra(self):
        ...
        return {
            ...
            "cos": (lambda l, i, o, para1, para2, para3, para4, para5, para6, para7: self.SynthCos(o)),
    ...
    def pickle_algebra(self):
        return {
            ...
            "sin": (lambda l, i, o, para1, para2, para3, para4, para5, para6, para7: (l, i, o)),

            "cos": (lambda l, i, o, para1, para2, para3, para4, para5, para6, para7: (l, i, o)),
...
if __name__ == "__main__":
    repo = ODE_3_Repository(linear_feature_dimensions=[1, 2, 5], constant_values=[0, 1, -1],
