...
class ODE_1_Repository:
    ...
    class Label(Group):
        ...
        def iter_linear(self):
            for in_f in self.linear_feature_dimensions:
                for out_f in self.linear_feature_dimensions:
                    yield ODE_1_Repository.Linear(in_features=in_f, out_features=out_f, bias=True)
                    yield ODE_1_Repository.Linear(in_features=in_f, out_features=out_f, bias=False)

        def iter_sigmoid(self):
            yield ODE_1_Repository.Sigmoid()
        ...
        def __contains__(self, item):
            ...
            elif isinstance(item, ODE_1_Repository.Sigmoid):
                return True
            elif isinstance(item, ODE_1_Repository.ReLu):
                return isinstance(item.inplace, bool)
            elif isinstance(item, ODE_1_Repository.Tanh):
                return True
            elif isinstance(item, ODE_1_Repository.Sum):
    ...
    class LossFunction(Group):
        ...
        def iter_mseloss(self):
            yield ODE_1_Repository.MSEloss(reduction="mean")
            yield ODE_1_Repository.MSEloss(reduction="sum")
            #yield ODE_1_Repository.MSEloss(reduction="none")

        def iter_l1loss(self):
            yield ODE_1_Repository.L1Loss(reduction="mean")
            yield ODE_1_Repository.L1Loss(reduction="sum")
        ...
        def __contains__(self, value):
            return (value is None or ((isinstance(value, ODE_1_Repository.MSEloss) or isinstance(value, ODE_1_Repository.L1Loss))
    ...
    class Optimizer(Group):
        ...
        def __iter__(self):
            for lr in self.learning_rate_values:
                yield ODE_1_Repository.Adam(learning_rate=lr)

        def __contains__(self, value):
            return isinstance(value, ODE_1_Repository.Adam) and (value.learning_rate in self.learning_rate_values)
